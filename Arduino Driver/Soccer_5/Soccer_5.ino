...
boolean decodeMsg(byte* buff, HardwareSerial* serial){	
  byte* p = buff;
...
bool fillBuffer(HardwareSerial *serial){
 ...
 // could check for valid message: h,m,p,s ....
  ...
  return decodeMsg(buffer,serial);
}
...
void loop()
{
  ...
  while (Serial1.available()) { // USB
    handleSerial(&Serial1);
  }
  while (Serial2.available()) { // Bluetooth
    handleSerial(&Serial2);
  }
}
